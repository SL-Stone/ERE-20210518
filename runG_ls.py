...
from modelG_ls import subject_model,object_model,evaluate
...
if __name__ == '__main__':
    ...
    Input_size = 60
    num_layers = 1
    CHECKPOINT = torch.load('./data/'+dataset+'/all/paramsls_10.pkl',map_location=device)
    ...
    for epoch in range(EPOCH): # 遍历数据集14次
        ...
        torch.save(state, 'data/'+dataset+'/all/paramsls_' + str(epoch) + '.pkl')
        f = open(file, 'a+')
