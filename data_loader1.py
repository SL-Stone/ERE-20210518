    ...
    def __iter__(self): #返回所有批的输入id矩阵,标注矩阵，迭代器
        ...
        for idx in idxs:
            ...
            if s2ro_map:
                ...
                obj_heads, obj_tails = np.zeros((text_len, self.num_rels), dtype=np.float32), np.zeros((text_len, self.num_rels), dtype=np.float32)
                for ro in s2ro_map.get((sub_head, sub_tail), []): 
                    obj_heads[ro[0]][ro[2]] = 1
                    obj_tails[ro[1]][ro[2]] = 1
                #print (sub_heads[1].dtype) #float32
                ...
                if len(tokens_batch) == self.batch_size or idx == idxs[-1]:
                    ...
                    #print (sub_tails_batch.shape)
                    #print("obj_heads_batch")
                    obj_heads_batch = seq_padding(obj_heads_batch, np.zeros(self.num_rels))#[batch_size,text_len(seq_len一个句子的序列长度),num_rels]
                    #print (obj_heads_batch.shape)
                    obj_tails_batch = seq_padding(obj_tails_batch, np.zeros(self.num_rels))#[batch_size,text_len(seq_len一个句子的序列长度),num_rels]
                    sub_head_batch, sub_tail_batch = torch.tensor(sub_head_batch), torch.tensor(sub_tail_batch)  #[batch_size,1]
