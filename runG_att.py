...
from modelG_att import subject_model,object_model,evaluate
...
if __name__ == '__main__':
    ...
    #CHECKPOINT = torch.load('data/'+dataset+'/all/paramsatt_31.pkl',map_location=device)
    ...
    file="outg_att.txt" #保存每个epoch下模型的性能/store the performance of the whole model in every epoch
    ...
    for epoch in range(EPOCH): 
        ...
        for i, data in enumerate(data_loader1):
            ...
            obj_loss=loss_func(o,obj_label.float().cuda())
        ...
        torch.save(state, 'data/'+dataset+'/all/paramsatt_' + str(epoch) + '.pkl')
        f = open(file, 'a+')
