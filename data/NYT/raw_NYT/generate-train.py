...
def is_multi_label(triples, is_relation_first=False):  #同一对实体有多种关系
...
def load_data(in_file, word_dict, rel_dict, out_file, normal_file, epo_file, seo_file):
    with open(in_file, 'r') as f1, open(out_file, 'w') as f2, open(normal_file, 'w') as f3, open(epo_file, 'w') as f4, open(seo_file, 'w') as f5:
        ...
        for line in lines:
            ...
            print(len(spos))  #56195
            print(len(sents)) #56195
...
if __name__ == '__main__':
    file_name = 'nyt/train.json'
    output = 'new_train.json'
    output_normal = 'new_train_normal.json'
    output_epo = 'new_train_epo.json'
    output_seo = 'new_train_seo.json'
    with open('nyt/relations2id.json', 'r') as f1, open('nyt/words2id.json', 'r') as f2:
        rel2id = json.load(f1)
        words2id = json.load(f2)
    rel_dict = {j:i for i,j in rel2id.items()}  #id2relation
    word_dict = {j:i for i,j in words2id.items()}  #id2word
