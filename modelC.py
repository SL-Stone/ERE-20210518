...
class subject_model(nn.Module):
    def __init__(self,hidden_size,lstmnum_layers):
        ...
        self.output_size = 2
        self.bilstm1 = nn.LSTM(self.input_size, self.hidden_size,self.num_layers,batch_first=True,bidirectional=True)
        self.bilstm2 = nn.LSTM(self.hidden_size*2, self.hidden_size, self.num_layers, batch_first=True, bidirectional=True)   

        # 初始时间步和最终时间步的隐藏状态作为全连接层输入
        self.q = nn.Parameter(torch.Tensor(self.hidden_size * 2, self.hidden_size * 2)) #Q权值矩阵
        self.k = nn.Parameter(torch.Tensor(self.hidden_size * 2, self.hidden_size * 2))    #K权值矩阵        
        nn.init.uniform_(self.q, -0.1, 0.1) #初始化为均匀分布
        nn.init.uniform_(self.k, -0.1, 0.1)
        
        self.hidden2tag = nn.Linear(self.hidden_size*2, self.output_size)
        self.posb = nn.Sigmoid() #转换为概率       
    ...
    def forward(self, inputs):
        ...
        key = torch.tanh(torch.matmul(x1, self.k))
        ...
        scored_x = torch.bmm(att_score,x1)
...
def subj_feature(x):  #1修改
    seq, idxs = x #均为tensor
    idxs = idxs.int()
    batch_idxs = torch.arange(0,seq.size(0)).int()
...
def extract_triples(sub_model, obj_model, tokenizer, bert_model, text_in, id2rel, h_bar=0.5, t_bar=0.5):  #提取模型预测的关系三元组
    ...
    with torch.no_grad():    
        sub_heads_logits, sub_tails_logits = (sub_model(torch.tensor([embeddings_ids]).float())).chunk(2, 2)
    ...
    if subjects:
        ...
        sub_heads = torch.tensor(sub_heads)
        #print (sub_heads.dtype) #int64
        sub_tails = torch.tensor(sub_tails)
        with torch.no_grad():
            obj_heads_logits, obj_tails_logits = (obj_model(torch.tensor(embeddings_ids1).float(), sub_heads, sub_tails)).chunk(2, 2)
